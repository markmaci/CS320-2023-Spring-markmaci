...
val is_safe = fn (bd, i, j) =>
let
    val rows = List.tabulate(8, fn i => i)
    val cols = List.map (fn row => board_get(bd, row)) rows
in
    not (List.exists (fn (col, row) =>
        col = j orelse abs(col - j) = abs(row - i)) (cols ~~ rows))
end
